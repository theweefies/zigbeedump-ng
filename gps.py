...
class DataStream(object):
    ...
    def __init__(self):
        ...
        for package_name, dataset in self.packages.items():
            _emptydict = {key: None for key in dataset}
            setattr(self, package_name, _emptydict)

        self.DEVICES['devices'] = {key: None for key in self.packages['DEVICE']}  # How does multiple listed devices work?
    ...
    def unpack(self, gpsd_socket_response):
        ...
        try:
            ...
            for key in package.keys():
                package[key] = fresh_data.get(key, None)  # Restores 'n/a' if key is absent in the socket response
